...
import type { RegisterForm, FieldErrors } from '../types/auth';

// Validation
function validate(form: RegisterForm): FieldErrors<RegisterForm> {
...
export default function Register() {
  ...
  const [fieldErrors, setFieldErrors] = useState<FieldErrors<RegisterForm>>({});
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.detail) {
          setApiError(data.detail as string);
        } else {
          // Maps DRF field errors
          const mapped: FieldErrors<RegisterForm> = {};
      ...
      // If the backend returns tokens on registration, log in immediately.
    ...
    <div className="flex flex-1 text-left">
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 bg-[#0c0c10] border-r border-[#1a1a22] p-12">
        ...
        <div className="space-y-6">
          <h1
            className="font-bold text-4xl leading-tight text-[#f0ede5]"
      ...
      <div className="flex flex-1 items-center justify-center p-8 bg-[#08080a]">
        ...
        >
          <div className="mb-8">
            ...
            <h2
              className="text-2xl font-bold text-[#f0ede5] mb-1"
              style={{ fontFamily: "'Syne', sans-serif" }}
            >
              Create account
            </h2>
            <p className="text-xs text-[#787891]">
          ...
          <p className="text-xs text-[#787891] text-center">
            ...
            <Link to="/login" className="text-[#aaef5a] hover:underline">
              Sign in
            </Link>
          </p>
        </form>
...
function Field({
  id, label, type, name, placeholder, value, onChange, error, autoComplete,
}: FieldProps) {
  return (
